...
function AlumnoKeep (nombre,apellido,edad,busqueda,bootcamp) {
    ...
    this.getFechas = function(curso){
        ...
        for ( curso in this.bootcamp){
            if(this.bootcamp.hasOwnProperty(curso)){
                ...
                fechaInicio=`No puedo darte esa información. Revisa el listado de cursos${Object.keys(this.bootcamp)}`
            }
        };
    ...
    return `Mi nombre es ${this.nombre+' '+this.apellido} y estoy apuntado a los cursos ${Object.keys(this.bootcamp)}`;
    };
};
    ...
    let alberto= new AlumnoKeep('Alberto','Romero Serrano',38,true,'keepcoding');
